...
function Upload(params) {
    try {
        ...
        data, params.hitch, params.thiss, params.context, this.status
        this.onafterupload = function (data, hitch, thiss, ctx, status) {
            ctx.SuccessFileCount++;
            params.onafterupload && params.onafterupload.call(this, data, hitch, thiss, ctx, status);
            if (ctx.SuccessFileCount + ctx.FailedFileCount == ctx.files.length) {
                ctx.oncomplete(ctx.SuccessFileCount, ctx.FailedFileCount);
            }
        };
        ...
        this.onerror = function (data, hitch, thiss, ctx, status) {
            if (isNull(status)) {
                params.onerror && params.onerror.call(ctx, data);
                return;
            }
            ctx.FailedFileCount++;
            params.onerror && params.onerror.call(ctx, data, hitch, thiss, ctx, status);
            if (ctx.SuccessFileCount + ctx.FailedFileCount == ctx.files.length) {
                ctx.oncomplete(ctx.SuccessFileCount, ctx.FailedFileCount);
            }
        };
        this.onbefore = function (httpRequest, hitch, caller) {
            var fileUpload = httpRequest.upload || {};
            fileUpload.ref = caller;
            params.onbefore && params.onbefore.call(this, httpRequest, hitch, caller);
        };
